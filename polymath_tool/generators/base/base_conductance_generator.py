#!/usr/bin/env python3
"""
Base class for conductance unit generators.
"""
...
class BaseConductanceGenerator(ABC):
    """Base class for conductance unit generators."""
    
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base conductance generator.
        
        Args:
            subdirectory: The subdirectory within conductance/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.conductance.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "conductance" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for conductance units.
...
{imports_section}import com.measures.conductance.Siemens
import com.measures.conductance.UnitConductance
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitConductance<{unit_name}> {{
...
fun UnitConductance<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the conductance units.
