#!/usr/bin/env python3
"""
Base class for illuminance unit generators.
"""
...
class BaseIlluminanceGenerator(ABC):
    """Base class for illuminance unit generators."""
    
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base illuminance generator.
        
        Args:
            subdirectory: The subdirectory within illuminance/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.illuminance.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "illuminance" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for illuminance units.
...
{imports_section}import com.measures.illuminance.Lux
import com.measures.illuminance.UnitIlluminance
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitIlluminance<{unit_name}> {{
...
fun UnitIlluminance<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the illuminance units.
