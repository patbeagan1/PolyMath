#!/usr/bin/env python3
"""
Base class for absorbed dose unit generators.
"""
...
class BaseAbsorbedDoseGenerator(ABC):
    """Base class for absorbed dose unit generators."""
    
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base absorbed dose generator.
        
        Args:
            subdirectory: The subdirectory within absorbeddose/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.absorbeddose.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "absorbeddose" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for absorbed dose units.
...
{imports_section}import com.measures.absorbeddose.Gray
import com.measures.absorbeddose.UnitAbsorbedDose
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitAbsorbedDose<{unit_name}> {{
...
fun UnitAbsorbedDose<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the absorbed dose units.
