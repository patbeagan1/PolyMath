#!/usr/bin/env python3
"""
Base class for activity unit generators.
"""
...
class BaseActivityGenerator(ABC):
    ...
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base activity generator.
        
        Args:
            subdirectory: The subdirectory within activity/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.activity.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "activity" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for activity units.
...
{imports_section}import com.measures.activity.Becquerel
import com.measures.activity.UnitActivity
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitActivity<{unit_name}> {{
...
fun UnitActivity<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the activity units.
