#!/usr/bin/env python3
"""
Base class for dose equivalent unit generators.
"""
...
class BaseDoseEquivalentGenerator(ABC):
    """Base class for dose equivalent unit generators."""
    
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base dose equivalent generator.
        
        Args:
            subdirectory: The subdirectory within doseequivalent/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.doseequivalent.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "doseequivalent" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for dose equivalent units.
...
{imports_section}import com.measures.doseequivalent.Sievert
import com.measures.doseequivalent.UnitDoseEquivalent
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitDoseEquivalent<{unit_name}> {{
...
fun UnitDoseEquivalent<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the dose equivalent units.
