#!/usr/bin/env python3
"""
Base class for luminous flux unit generators.
"""
...
class BaseLuminousFluxGenerator(ABC):
    """Base class for luminous flux unit generators."""
    
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base luminous flux generator.
        
        Args:
            subdirectory: The subdirectory within luminousflux/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.luminousflux.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "luminousflux" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for luminous flux units.
...
{imports_section}import com.measures.luminousflux.Lumen
import com.measures.luminousflux.UnitLuminousFlux
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitLuminousFlux<{unit_name}> {{
...
fun UnitLuminousFlux<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the luminous flux units.
