#!/usr/bin/env python3
"""
Base class for catalytic activity unit generators.
"""
...
class BaseCatalyticActivityGenerator(ABC):
    """Base class for catalytic activity unit generators."""
    
    def __init__(self, subdirectory: str, package_name: str):
        """
        Initialize the base catalytic activity generator.
        
        Args:
            subdirectory: The subdirectory within catalyticactivity/ (e.g., "metric", "customary")
            package_name: The Kotlin package name (e.g., "com.measures.catalyticactivity.metric")
        """
        self.subdirectory = subdirectory
        self.package_name = package_name
        self.base_dir = get_measures_base() / "catalyticactivity" / subdirectory
    ...
    def get_template(self) -> str:
        """
        Get the Kotlin template for catalytic activity units.
...
{imports_section}import com.measures.catalyticactivity.Katal
import com.measures.catalyticactivity.UnitCatalyticActivity
import kotlin.jvm.JvmInline

@JvmInline
value class {unit_name}(override val value: Double) : UnitCatalyticActivity<{unit_name}> {{
...
fun UnitCatalyticActivity<*>.to{unit_name}() = toUnit({unit_name}(1.0))
"""
    
    def generate(self) -> int:
        """
        Generate the catalytic activity units.
